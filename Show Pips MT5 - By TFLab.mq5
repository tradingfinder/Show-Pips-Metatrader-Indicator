...
input color ProfitColor = clrDodgerBlue;
      ...
      for(int i=PositionsTotal()-1; i>=0; i--)
        {
         ...
         double OpenPos = PositionGetDouble(POSITION_PRICE_OPEN);
         double CurrentPos = PositionGetDouble(POSITION_PRICE_CURRENT);

         if(PositionGetInteger(POSITION_TYPE) == POSITION_TYPE_BUY)
            Pips += int((CurrentPos-OpenPos)/Point());
         else
            Pips += int((OpenPos-CurrentPos)/Point());
         ...
         if(TextPosition == UpperLeft)
           ...
           {x=int(ChartGetInteger(0,CHART_WIDTH_IN_PIXELS,0))-600;y=10;}

         LabelCreate("Show Pips "+Symbol(),x,y,CORNER_LEFT_UPPER,"Current candle pip(s): " +IntegerToString(Pips)+"  ||  Time to close: "+TimeShow,"Arial",14,Clr);
        }
     }
